...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train chest X-ray abnormality classification model.")
    parser.add_argument(
        "--splits-dir",
        type=Path,
    ...
    label_to_idx = infer_label_map(train_csv, modality="xray")
    ...
    save_training_artifacts(
        ...
        label_to_idx=label_to_idx,
        history=history,
        model_path=args.artifact_dir / "xray_model.pt",
        label_map_path=args.artifact_dir / "label_map_xray.json",
        metrics_path=args.artifact_dir / "metrics_xray.json",
    )
